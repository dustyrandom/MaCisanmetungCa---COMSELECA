    ...
    return (
      <div key={s.id} className="border rounded-lg p-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <span
            className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-full uppercase ${
              s.status === 'approved'
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          ...
          {/* Approve - only show if not yet approved or rejected */}
          {s.status === 'pending' && (
            <button
              disabled={savingId === s.id}
              onClick={() => {
          ...
          {/* Reject - only show if not yet approved or rejected */}
          {s.status === 'pending' && (
            <button
              disabled={savingId === s.id}
              onClick={() => {
